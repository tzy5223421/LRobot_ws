  ...
  struct Cia402Data
  {
    ...
    bool init_data(hardware_interface::ComponentInfo &joint, std::string device_dump)
    {
      ...
      if (config["position_mode"])
      {
        ...
        RCLCPP_INFO(rclcpp::get_logger(joint_name), "Registered position_mode %s", "isComplete");
      }
    ...
    void export_state_interface(std::vector<hardware_interface::StateInterface> &state_interfaces)
    {
      ...
      state_interfaces.emplace_back(hardware_interface::StateInterface(
          joint_name, "actual_position", &actual_position));

      // actual speed
      state_interfaces.emplace_back(hardware_interface::StateInterface(
          joint_name, "actual_velocity", &actual_velocity));
    }

    void export_command_interface(
        std::vector<hardware_interface::CommandInterface> &command_interfaces)
    {
      if (command_interface_to_operation_mode.find(joint_name + "/" + "poisition") != command_interface_to_operation_mode.end())
      {
        // target position
        RCLCPP_INFO(rclcpp::get_logger("export_command_interface"), "Registered export_command_interface %s", joint_name.c_str());
        command_interfaces.emplace_back(joint_name, "actual_poisition", &target_position);
        interfaces.push_back(joint_name + "/" + "actual_poisition");
      }
      if (command_interface_to_operation_mode.find(joint_name + "/" + "velocity") != command_interface_to_operation_mode.end())
      {
        // target velocity
        command_interfaces.emplace_back(joint_name, "actual_velocity", &target_velocity);
        interfaces.push_back(joint_name + "/" + "actual_velocity");
      }
      if (command_interface_to_operation_mode.find(joint_name + "/" + "effort") != command_interface_to_operation_mode.end())
      {
        // target effort
